...
let largeShirts = [];
largeShirts.push(runners.filter(el => el.shirt_size === "L"));
console.log(largeShirts);

// ==== Challenge 4: Use .reduce() ====
// The donations need to be tallied up and reported for tax purposes. Add up all the donations into a ticketPriceTotal array and log the result
let ticketPriceTotal = [];
ticketPriceTotal.push(runners.reduce(function(acc, currVal) {
...
newRunners.map(el => el.donation += (Math.floor(Math.random() * 300) + 20))
console.log(newRunners);


// Problem 2
let largeDonationsSorted = [];
runners.filter(val => val.donation > 200).sort((a, b) => a.donation - b.donation).reverse().forEach(function(el) {
    largeDonationsSorted.push(`${el.first_name} ${el.last_name}: $${el.donation}`);
});
console.log(largeDonationsSorted);

// Problem 3
let averageDonation = 0;
averageDonation = runners.reduce(function(acc, currVal){
    return (acc + currVal.donation);
}, 0)
